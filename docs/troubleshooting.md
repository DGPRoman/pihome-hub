...
| `… declares workshop-pc, but PIHOME_DEVICE_API_KEY is not set` | Devices are declared with no key for them to announce with, so every announcement would be refused | Generate a third key into `/etc/pihome-hub/hub.env`, or remove the devices |
